...
func (d *DB) runCompaction(
	...
	finishOutput := func(splitKey []byte) error {
		...
		truncateTombstones := splitL0Outputs || !invariants.Enabled || !disableTombstoneTruncation(c, splitKey)
		for _, v := range iter.Tombstones(splitKey, truncateTombstones) {
			if tw == nil {
		...
		// If we did not truncate range tombstones when we emitted them, we need
		// to truncate FileMetadata.LargestRangeDel so that this file doesn't
		// overlap the next file.
		// NB: In production, we currently always truncate range tombstones.
		// This code exists for exercising existing untruncated range tombstones
		// in test cases.
		if !truncateTombstones && splitKey != nil && writerMeta.LargestRangeDel.UserKey != nil {
			// The current file is not the last output file and there is a range
			// tombstone in it.  If the tombstone extends into the next file,
			// then truncate it for the purposes of computing meta.Largest. For
			// example, say the next file's first key is c#7,1 and the current
			// file's last key is c#10,1 and the current file has a range
			// tombstone [b, d)#12,15. For purposes of the bounds we pretend
			// that the range tombstone ends at c#inf where inf is the
			// InternalKeyRangeDeleteSentinel. Note that this is just for
			// purposes of bounds computation -- the current sstable will end up
			// with a Largest key of c#7,1 so the range tombstone in the current
			// file will be able to delete c#7.
...
// disableTombstoneTruncation is used to deterministically disable truncation of
// range tombstones in tests.
func disableTombstoneTruncation(c *compaction, splitKey []byte) bool {
	// Fibonacci hash https://probablydance.com/2018/06/16/fibonacci-hashing-the-optimization-that-the-world-forgot-or-a-better-alternative-to-integer-modulo/
	simpleHash := (11400714819323198485 * uint64(uintptr(unsafe.Pointer(c)))) >> 63
	return splitKey != nil && splitKey[0]&byte(1) == 0 && simpleHash == 0
}
